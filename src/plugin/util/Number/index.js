...
function m() {
  let ext = {
    ...
    term: function (number, o) {
      let formatStr, split, char, fixed, fixedMode, arr, integer, decimal;
      ...
      if (o.force) {// 小数部分强制保留和fixed相同的位数，不足补0
        for (let l = decimal.length; l < fixed; l++) {
          decimal += "0";
        }
      }
    ...
    determ: function (str, char) {
      let reg = char instanceof RegExp ? char : new RegExp(char || ",", "g");
